...
public class BoardDBBean {
    ...
    public void insertArticle(BoardDataBean article) 
        ...
        try {
		    ...
		    if (num!=0) {  
		      sql="update QAboard set re_step=re_step+1 ";
            ...
            // 쿼리를 작성
            sql = "insert into QAboard(writer,email,subject,passwd,reg_date,";
	...
	public List<BoardDataBean> getArticles(int start, int end)
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            	"select * from QAboard order by ref desc, re_step asc limit ?,? ");
	...
	public BoardDataBean getArticle(int num)
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            	"update QAboard set readcount=readcount+1 where num = ?");
			pstmt.setInt(1, num);
			pstmt.executeUpdate();

            pstmt = conn.prepareStatement(
            	"select * from QAboard where num = ?");
            pstmt.setInt(1, num);
            rs = pstmt.executeQuery();
        ...
        try {
			...
			if(rs.next()){
			  ...
			  if(dbwriter.equals(article.getWriter())){
                sql="update QAboard set subject=?";
    ...
    public int deleteArticle(int num, String name)
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            	"select writer from QAboard where num = ?");
            pstmt.setInt(1, num);
            rs = pstmt.executeQuery();
    ...
    public List<BoardDataBean> selectArticle(String num, String ab)
        ...
        try {
          ...
          if(ab.equals("title")) {
        	  sql="select * from QAboard where subject like ?";
            
              pstmt = conn.prepareStatement(sql);
              pstmt.setString(1, "%"+num+"%");
          }
          else if(ab.equals("content")){
        	  	sql="select * from QAboard where content like ?";
              
              pstmt = conn.prepareStatement(sql);
              pstmt.setString(1, "%"+num+"%");
          }
          else if(ab.equals("writer")){
        	  sql="select * from QAboard where writer like ?";
              
              pstmt = conn.prepareStatement(sql);
              pstmt.setString(1, "%"+num+"%");
          }
          else if(ab.equals("all")) {
        	  sql="select * from QAboard where writer like ? or subject like ? or content like ?";
